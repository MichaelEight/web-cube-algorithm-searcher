...
export function simplifySequence(cube: CubeSpec, moves: readonly Move[]): Move[] {
  ...
  while (changed) {
    ...
    for (let i = 0; i < out.length - 1; i++) {
      ...
      if (newName === null) {
        ...
        out.splice(i, 2, cube.MOVE_REGISTRY[newName])
      }
      changed = true
      break
    }
...
export function simplifyAndDedupe(cube: CubeSpec, solutions: readonly (readonly Move[])[]): Move[][] {
  const seen = new Set<string>()
  const out: Move[][] = []
  for (const s of solutions) {
    const simplified = simplifySequence(cube, s)
